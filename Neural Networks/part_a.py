...
def init_network(M_data, num_labels, num_hidden_layers, hidden_layer_sizes, activation):
	...
	if(L != 0):
		...
		W[L+1] = np.random.randn(hidden_layer_sizes[L-1], num_labels)*np.sqrt(1.0/hidden_layer_sizes[L-1])	#since output layer is necessarily sigmoid
		b[L+1] = np.zeros((1, num_labels))
	else:
		W[1] = np.random.randn(M_data, num_labels)*np.sqrt(1.0/M_data)	#since output layer is necessarily sigmoid
		b[1] = np.zeros((1, num_labels))
...
def backwardpropagation(A, W, num_hidden_layers, num_examples, Y_data, activation):
	...
	dZ = (2*(A[L+1] - Y_data)*(derivative_activate(A[L+1], 'sigmoid')))/(num_examples)
...
def loss_function(Y_pred, Y, num_examples):
	error = Y_pred - Y
	loss = (np.sum(error*error))/num_examples
	return loss
...
def main():
	...
	initial_lr = float(sys.argv[5])
	activation = sys.argv[6]
	hidden_layer_sizes = [int(x) for x in sys.argv[7:]]
	...
	num_epochs = 300	#This is being set beforehand
