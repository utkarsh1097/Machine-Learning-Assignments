...
def constant_learning_rate(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters = 500):
	best_lambd = -1
	min_error = 100000000
	...
	for lambd in lambd_list:
		...
		for i in range(10):
			...
			num_batches = N_train//batch_size	#One epoch covers num_batches batch sizes.Do this till 
			cur_idx = 0

			#Now train for num_iter iterations on the train data 
			for j in range(num_iters):
				if(cur_idx+batch_size >= N_train):
					X_train_batch = vstack((X_train[cur_idx:N_train], X_train[0:batch_size-N_train+cur_idx]))
					Y_train_batch = np.append(Y_train[cur_idx:N_train], Y_train[0:cur_idx+batch_size-N_train], axis = 0)
					cur_idx = cur_idx+batch_size-N_train
				else:
					X_train_batch = X_train[cur_idx:cur_idx+batch_size]
					Y_train_batch = Y_train[cur_idx:cur_idx+batch_size]
					cur_idx = cur_idx+batch_size

				weights = gradient_descent(weights, X_train_batch, Y_train_batch, M_data, alpha, lambd)	#After 500 steps of iterations
...
def adaptive_learning_rate(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters = 500):
	best_lambd = -1
	min_error = 100000000
	...
	for lambd in lambd_list:
		...
		for i in range(10):
			...
			for j in range(num_iters):
				...
				weights = gradient_descent(weights, X_train_batch, Y_train_batch, M_data, (alpha)/math.sqrt(j+1), lambd)	#After 500 steps of iterations
...
def exact_line_search(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters = 100):
	best_lambd = -1
	min_error = 100000000
	...
	for lambd in lambd_list:
		...
		for i in range(10):
			...
			for j in range(num_iters):
				...
				pred_error = predictions(weights, X_train_batch) - Y_train_batch
				d = calc_gradient(X_train_batch, pred_error) #vector along optimum

				low_alpha = 0	#Lower bound of alpha
				high_alpha = alpha 	#Upperbound of alpha
				while(derivative_f(high_alpha, d, X_train_batch, pred_error) < 0):
				...
				weights = gradient_descent(weights, X_train_batch, Y_train_batch, M_data, use_alpha, lambd)
...
def pred_constant_learning_rate(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters = 500):
	weights = np.zeros((M_data, 1))	#Initialize

	N_data = Y_data.shape[0]
	num_batches = N_data//batch_size	#One epoch covers num_batches batch sizes.Do this till 
	cur_idx = 0

	for i in range(num_iters):
		if(cur_idx+batch_size >= N_data):
			X_data_batch = vstack((X_data[cur_idx:N_data], X_data[0:batch_size-N_data+cur_idx]))
			Y_data_batch = np.append(Y_data[cur_idx:N_data], Y_data[0:cur_idx+batch_size-N_data], axis = 0)
			cur_idx = cur_idx+batch_size-N_data
		else:
			X_data_batch = X_data[cur_idx:cur_idx+batch_size]
			Y_data_batch = Y_data[cur_idx:cur_idx+batch_size]
			cur_idx = cur_idx+batch_size
		weights = gradient_descent(weights, X_data_batch, Y_data_batch, M_data, alpha, best_lambd)
...
def pred_adaptive_learning_rate(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters = 500):
	weights = np.zeros((M_data, 1))	#Initialize
	...
	for i in range(num_iters):
		...
		weights = gradient_descent(weights, X_data_batch, Y_data_batch, M_data, alpha/np.sqrt(i+1), best_lambd)
...
def pred_exact_line_search(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters = 100):
	weights = np.zeros((M_data, 1))	#Initialize
	...
	for i in range(num_iters):
		...
		pred_error = predictions(weights, X_data_batch) - Y_data_batch
		d = calc_gradient(X_data_batch, pred_error)
		
		low_alpha = 0	#Lower bound of alpha
		high_alpha = alpha 	#Upperbound of alpha

		while(derivative_f(high_alpha, d, X_data_batch, pred_error) < 0):
			high_alpha = 2*high_alpha
		...
		weights = gradient_descent(weights, X_data_batch, Y_data_batch, M_data, use_alpha, best_lambd)
...
def main():
	...
	if(method == '1'):	#Constant learning rate
		best_lambd = constant_learning_rate(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters)
		Y_pred = pred_constant_learning_rate(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters)
		...
		best_lambd = adaptive_learning_rate(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters)
		Y_pred = pred_adaptive_learning_rate(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters)
		...
		best_lambd = exact_line_search(lambd_list, X_data, Y_data, M_data, N_data, alpha, batch_size, num_iters)
		Y_pred = pred_exact_line_search(best_lambd, X_data, Y_data, X_test, M_data, alpha, batch_size, num_iters)
